...
export default function AnalysisVisualizer({ sections, crediScore }: AnalysisVisualizerProps) {
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border border-gray-200 rounded-lg">
              <thead className="bg-gray-50">
                <tr>
                  {allKeys.map(key => (
                    <th key={key} className="px-4 py-2 text-left text-sm font-medium text-gray-900 border-b border-gray-200">
              ...
              <tbody>
                {data.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    {allKeys.map(key => (
                      <td key={key} className="px-4 py-2 text-sm text-gray-700 border-b border-gray-200">
      ...
      return (
        <div className="space-y-4">
          {Object.entries(data).map(([key, value]) => {
            ...
            const HeadingTag = depth === 0 ? 'h3' : depth === 1 ? 'h4' : depth === 2 ? 'h5' : 'h6';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Credibility Analysis Results</h2>
            <p className="text-gray-600">Based on comprehensive content evaluation</p>
          </div>
          <div className={`text-center p-6 rounded-xl ${getScoreBackground(crediScore)} border-2 border-opacity-20`}>
            <div className="text-sm font-medium text-gray-600 mb-1">Credi Score</div>
