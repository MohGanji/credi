...
export function generateMockPosts(platform: 'twitter' | 'linkedin', count: number = 50): MockPost[] {
  const templates = platform === 'twitter' ? twitterPostTemplates : linkedinPostTemplates;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const links: string[] = [];
    if (Math.random() > 0.7) { // 30% chance of having links
      const linkCount = Math.floor(Math.random() * 2) + 1;
      for (let j = 0; j < linkCount; j++) {
        links.push(`https://example.com/article-${Math.floor(Math.random() * 1000)}`);
      }
    }
    
    // Generate realistic engagement metrics based on platform
    const baseEngagement = platform === 'twitter' ? 
...
export function generateMockProfileData(platform: 'twitter' | 'linkedin', username: string): MockProfileData {
  const isTwitter = platform === 'twitter';
  
  return {
    platform,
    username,
    displayName: isTwitter ? `@${username}` : username.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
    bio: isTwitter ? 
...
export function generateMockAnalysisSections(platform: 'twitter' | 'linkedin', username: string, posts: MockPost[]) {
  ...
  const totalEngagement = posts.reduce((sum, post) => 
    sum + post.engagementMetrics.likes + post.engagementMetrics.shares + post.engagementMetrics.comments, 0);
  ...
  return {
    overview: {
      ...
      data: {
        ...
        "Profile Status": profileData.verified ? "Verified and Active" : "Active",
    ...
    strengths: {
      ...
      data: {
        ...
        "Content Depth": "Posts demonstrate thoughtful analysis rather than surface-level commentary",
    ...
    criteria_evaluation: {
      ...
      data: {
        "Unnecessary Complexity": {
          ...
          evaluation: "Uses clear, accessible language appropriate for the platform audience"
        },
        "Proprietary/Pushy Selling": {
          status: Math.random() > 0.5 ? "pass" as const : "warning" as const,
        ...
        "Us vs. Them Framing": {
          status: "pass" as const,
          evaluation: "Promotes collaborative and inclusive discourse without divisive language"
        },
        "Overselling Narrow Interventions": {
          status: "pass" as const,
          evaluation: "Presents balanced perspectives without overpromising outcomes"
        },
        "Emotion/Story vs Data": {
          status: Math.random() > 0.3 ? "pass" as const : "warning" as const,
          evaluation: Math.random() > 0.3 ?
        ...
        "Lack of Sourcing": {
          status: linkPercentage > 20 ? "pass" as const : "warning" as const,
          evaluation: linkPercentage > 20 ?
        ...
        "Serial Contrarian": {
          status: "pass" as const,
          evaluation: "Aligns with industry best practices and established knowledge"
        },
        "Guru Syndrome": {
          status: "pass" as const,
          evaluation: "Demonstrates humility and acknowledges contributions of others in the field"
        }
      }
    },
    representative_posts: {
      ...
      data: {
        "Recent Highlights": posts.slice(0, 3).map(post => ({
          content: post.content.length > 150 ? post.content.substring(0, 150) + "..." : post.content,
          timestamp: post.timestamp.toLocaleDateString(),
          url: "#", // Mock URL
          reasoning: "Demonstrates thoughtful analysis and professional expertise"
        })),
        "High Engagement": posts
          .sort((a, b) => (b.engagementMetrics.likes + b.engagementMetrics.shares + b.engagementMetrics.comments) - 
                          (a.engagementMetrics.likes + a.engagementMetrics.shares + a.engagementMetrics.comments))
          .slice(0, 2)
          .map(post => ({
            content: post.content.length > 150 ? post.content.substring(0, 150) + "..." : post.content,
            timestamp: post.timestamp.toLocaleDateString(),
    ...
    score_justification: {
      ...
      data: {
        "Why Not Higher": [
          linkPercentage < 30 ? "Could improve external sourcing and citation practices" : null,
          avgEngagement < 50 ? "Engagement levels could be higher for the follower count" : null,
        ...
        "Why Not Lower": [
          ...
          profileData.verified ? "Verified account adds credibility" : "Authentic engagement patterns"
        ],
        "Key Factors": [
...
export function generateMockAnalysisResult(profileUrl: string, platform: 'twitter' | 'linkedin', username: string) {
