...
test('Retry Integration Tests', async (t) => {
  ...
  t.test('should handle complete retry workflow', async (t) => {
    ...
    const failedAnalysis = await AnalysisRepository.createWithState(analysisData, 'ANALYSIS_FAILED');
    await AnalysisRepository.update(failedAnalysis.id, {
      errorMessage: 'AI parsing failed',
      retryCount: 1,
    });

    t.equal(failedAnalysis.state, 'ANALYSIS_FAILED', 'Analysis should be in failed state');
    t.ok(failedAnalysis.postsId, 'Analysis should have posts reference');

    // Step 2: Simulate retry logic
    const latestFailed = await AnalysisRepository.findLatestFailedByProfileUrl(testProfileUrl);
    t.ok(latestFailed, 'Should find latest failed analysis');
    t.equal(latestFailed?.id, failedAnalysis.id, 'Should return the correct failed analysis');
    ...
    if (latestFailed && canRetry) {
      ...
      const updatedAnalysis = await AnalysisRepository.updateState(latestFailed.id, 'ANALYZING');
      t.equal(updatedAnalysis.state, 'ANALYZING', 'Should update to analyzing state');

      // Increment retry count
      await AnalysisRepository.incrementRetryCount(latestFailed.id);

      // Simulate successful completion
      const completedAnalysis = await AnalysisRepository.update(latestFailed.id, {
        state: 'COMPLETED',
        crediScore: 7.5,
        sections: [
          { name: 'Test Section', data: { score: 7.5, analysis: 'Test analysis' } },
        ],
      });

      t.equal(completedAnalysis.state, 'COMPLETED', 'Should complete successfully');
      t.equal(completedAnalysis.crediScore, 7.5, 'Should have updated score');
      t.equal(completedAnalysis.retryCount, 2, 'Should have incremented retry count');
    }
  });
  ...
  t.test('should prevent retry when max attempts reached', async (t) => {
    ...
    const maxRetryAnalysis = await AnalysisRepository.createWithState(analysisData, 'ANALYSIS_FAILED');
    await AnalysisRepository.update(maxRetryAnalysis.id, {
      retryCount: 3, // Max retries reached
      errorMessage: 'Max retries reached',
    });

    const latestFailed = await AnalysisRepository.findLatestFailedByProfileUrl(maxRetryUrl);
    const canRetry = (latestFailed?.retryCount || 0) < 3;
  ...
  t.test('should handle crawling failure retry', async (t) => {
    ...
    const crawlingFailedAnalysis = await AnalysisRepository.createWithState(analysisData, 'CRAWLING_FAILED');
    await AnalysisRepository.update(crawlingFailedAnalysis.id, {
      errorMessage: 'Crawler failed to fetch profile',
      retryCount: 1,
    });

    t.equal(crawlingFailedAnalysis.state, 'CRAWLING_FAILED', 'Analysis should be in crawling failed state');
    t.notOk(crawlingFailedAnalysis.postsId, 'Analysis should not have posts when crawling failed');

    // Simulate retry from crawling
    const latestFailed = await AnalysisRepository.findLatestFailedByProfileUrl(crawlingFailUrl);
    t.ok(latestFailed, 'Should find latest failed analysis');
    t.equal(latestFailed?.state, 'CRAWLING_FAILED', 'Should be in crawling failed state');

    // Update to crawling state for retry
    const retryAnalysis = await AnalysisRepository.updateState(latestFailed!.id, 'CRAWLING');
    t.equal(retryAnalysis.state, 'CRAWLING', 'Should update to crawling state for retry');
  });
  ...
  t.test('should reuse valid posts for analysis retry', async (t) => {
    ...
    const postsData = {
      ...
      followerCount: 1000,
      posts: [
        { id: '1', content: 'Reusable post 1', timestamp: new Date().toISOString() },
        { id: '2', content: 'Reusable post 2', timestamp: new Date().toISOString() },
        { id: '3', content: 'Reusable post 3', timestamp: new Date().toISOString() },
      ],
    };
    ...
    const validPosts = await PostsRepository.findValidPostsByProfileUrl(reusePostsUrl);
    t.ok(validPosts, 'Should find valid posts for reuse');
    t.equal(validPosts?.id, posts.id, 'Should return the same posts record');
    t.equal((validPosts?.posts as any[]).length, 3, 'Should have all posts available');

    // Verify analysis references the posts
    t.equal(failedAnalysis.postsId, posts.id, 'Analysis should reference the posts');
  });
  ...
  t.test('should handle expired posts scenario', async (t) => {
    ...
    const expiredPostsData = {
      ...
      followerCount: 200,
      posts: [
        { id: '1', content: 'Expired post 1', timestamp: new Date().toISOString() },
      ],
    };
