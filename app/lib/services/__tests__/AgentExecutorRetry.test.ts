...
import { z } from 'zod';
import { AgentExecutorService, ModelConfig, AgentConfig } from '../AgentExecutorService';
    ...
    return {
      invoke: async (messages: any[]) => {
        ...
        if (this.failureCount <= this.maxFailures) {
          throw new Error(`Mock structured output failure ${this.failureCount}`);
        }
...
test('AgentExecutorService retry helper methods', async (t) => {
  ...
  t.test('should add retry instructions to prompt', async (t) => {
    const addRetryInstructions = (service as any).addRetryInstructions.bind(service);
