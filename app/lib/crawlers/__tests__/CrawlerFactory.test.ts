...
describe('CrawlerFactory', () => {
  describe('fetchProfile', () => {
    it('should successfully fetch Twitter profiles', async () => {
      const profile = await CrawlerFactory.fetchProfile(
    ...
    it('should throw error for unsupported URLs', async () => {
      await expect(
        CrawlerFactory.fetchProfile('https://facebook.com/user')
      ).rejects.toThrow('Unsupported platform');
    });

    it('should throw error for malformed URLs', async () => {
      await expect(CrawlerFactory.fetchProfile('not-a-url')).rejects.toThrow(
        'Unsupported platform'
      );
    });

    it('should support both twitter.com and x.com domains', async () => {
      const twitterProfile = await CrawlerFactory.fetchProfile(
        'https://twitter.com/testuser'
      );
      const xProfile = await CrawlerFactory.fetchProfile(
  ...
  describe('fetchRecentPosts', () => {
    it('should successfully fetch Twitter posts', async () => {
      const posts = await CrawlerFactory.fetchRecentPosts(
        'https://twitter.com/testuser',
        10
      );
    ...
    it('should respect maxCount parameter', async () => {
      const posts3 = await CrawlerFactory.fetchRecentPosts(
        'https://twitter.com/testuser',
        3
      );
      const posts10 = await CrawlerFactory.fetchRecentPosts(
        'https://twitter.com/testuser2',
        10
      );
    ...
    it('should throw error for invalid maxCount', async () => {
      await expect(
        CrawlerFactory.fetchRecentPosts('https://twitter.com/testuser', 0)
      ).rejects.toThrow('maxCount must be between 1 and 100');

      await expect(
        CrawlerFactory.fetchRecentPosts('https://twitter.com/testuser', 101)
      ).rejects.toThrow('maxCount must be between 1 and 100');
    });

    it('should throw error for unsupported URLs', async () => {
      await expect(
        CrawlerFactory.fetchRecentPosts('https://facebook.com/user', 10)
      ).rejects.toThrow('Unsupported platform');
    });
  });
  ...
  describe('Platform support', () => {
    ...
    it('should check if URL is supported', () => {
      ...
      expect(CrawlerFactory.isSupported('https://linkedin.com/in/user')).toBe(
        true
      );
      expect(CrawlerFactory.isSupported('https://x.com/user')).toBe(true);
      expect(CrawlerFactory.isSupported('https://facebook.com/user')).toBe(
        false
      );
  ...
  describe('Error handling', () => {
    it('should handle rate limiting', async () => {
      ...
      const promises = Array.from(
        { length: 15 },
        (_, i) =>
          CrawlerFactory.fetchProfile(`https://twitter.com/ratetest${i}`).catch(
            (error) => error
