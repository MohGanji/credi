...
export class TwitterCrawler extends BaseCrawler {
  ...
  protected async scrapePosts(url: string, maxCount: number): Promise<Post[]> {
    ...
    for (let i = 0; i < actualCount; i++) {
      ...
      const createdAt = new Date(Date.now() - (i * 3600000) - Math.random() * 3600000).toISOString(); // Random times in past hours
  ...
  private generateDisplayName(username: string): string {
    const names = [
      ...
      'Marcus Williams', 'Lisa Zhang', 'James Martinez', 'Rachel Green', 'Kevin Park'
    ];
  ...
  private generateRandomUsername(): string {
    const usernames = ['techguru', 'codemaster', 'devlife', 'buildthings', 'innovator', 'creator', 'maker', 'hacker'];
  ...
  private generateRandomDisplayName(): string {
    const names = ['Sam Wilson', 'Jordan Lee', 'Taylor Swift', 'Morgan Davis', 'Casey Johnson'];
