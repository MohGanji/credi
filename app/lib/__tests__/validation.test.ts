...
test('detectPlatform - should detect LinkedIn platform', (t) => {
  ...
  linkedinUrls.forEach((url) => {
    t.equal(
      detectPlatform(url),
      'linkedin',
      `Should detect LinkedIn for ${url}`
    );
  });
...
test('extractUsername - should extract username from Twitter URLs', (t) => {
  t.equal(
    extractUsername('https://twitter.com/username', 'twitter'),
    'username'
  );
  t.equal(extractUsername('https://x.com/username/', 'twitter'), 'username');
  t.equal(
    extractUsername('https://twitter.com/user_name', 'twitter'),
    'user_name'
  );
  t.end();
});
...
test('extractUsername - should extract username from LinkedIn URLs', (t) => {
  t.equal(
    extractUsername('https://linkedin.com/in/username', 'linkedin'),
    'username'
  );
  t.equal(
    extractUsername('https://linkedin.com/in/username/', 'linkedin'),
  ...
  t.equal(
    extractUsername('https://linkedin.com/in/user-name', 'linkedin'),
    'user-name'
  );
  t.end();
});
...
test('extractUsername - should throw error for invalid URLs', (t) => {
  t.throws(
    () => extractUsername('not-a-url', 'twitter'),
  ...
  t.throws(
    ...
    'Should throw for wrong platform'
  );
  t.end();
});
...
test('normalizeUrl - should normalize Twitter URLs to x.com', (t) => {
  t.equal(
    normalizeUrl('https://twitter.com/username', 'twitter'),
    'https://x.com/username'
  );
  t.equal(
    normalizeUrl('https://x.com/username/', 'twitter'),
    'https://x.com/username'
  );
  t.equal(
    normalizeUrl('http://www.twitter.com/username', 'twitter'),
    'https://x.com/username'
  );
  t.end();
});
...
test('normalizeUrl - should normalize LinkedIn URLs', (t) => {
  t.equal(
    normalizeUrl('https://linkedin.com/in/username', 'linkedin'),
    'https://linkedin.com/in/username'
  );
  t.equal(
    normalizeUrl('https://linkedin.com/in/username/', 'linkedin'),
  ...
  t.equal(
    normalizeUrl('http://www.linkedin.com/in/user-name', 'linkedin'),
    'https://linkedin.com/in/user-name'
  );
  t.end();
});
...
test('normalizeUrl - should throw error for invalid inputs', (t) => {
  t.throws(
    () => normalizeUrl('not-a-url', 'twitter'),
    'Should throw for invalid URL'
  );
  t.end();
});
...
test('validateSocialMediaUrl - should validate and return platform for valid URLs', (t) => {
  ...
  testCases.forEach(({ url, expectedPlatform }) => {
    ...
    t.equal(
      result.platform,
      expectedPlatform,
      `Should detect ${expectedPlatform} for ${url}`
    );
...
test('validateSocialMediaUrl - should handle invalid inputs', (t) => {
  const invalidInputs = [
    ...
    {
      input: null as any,
      expectedError: 'URL is required and must be a string',
    },
    {
  ...
  invalidInputs.forEach(({ input, expectedError }) => {
    ...
    t.equal(
      result.error,
      expectedError,
      `Should have correct error for ${input}`
    );
...
test('validateSocialMediaUrl - should reject unsupported platforms', (t) => {
  ...
  unsupportedUrls.forEach((url) => {
    ...
    t.equal(
      ...
      `Should have correct error for ${url}`
    );
...
test('isPlatformSupported - should return false for unsupported platforms', (t) => {
  t.equal(
    isPlatformSupported('facebook'),
    false,
    'Should not support Facebook'
  );
  t.equal(
    isPlatformSupported('instagram'),
    false,
    'Should not support Instagram'
  );
  ...
  t.equal(
    isPlatformSupported('invalid'),
    false,
    'Should not support invalid platform'
  );
  t.end();
});
