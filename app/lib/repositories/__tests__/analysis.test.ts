...
const mockAnalysisData: CreateAnalysisData = {
  ...
  sections: [
    ...
    },
    {
      ...
      data: {
        'Technical Expertise':
          'High technical knowledge in software development',
...
tap.test(
  'AnalysisRepository.create should create a new analysis',
  async (t) => {
    ...
    t.equal(
      analysis.profileUrl,
      mockAnalysisData.profileUrl,
      'Profile URL should match'
    );
    t.equal(
      analysis.platform,
      mockAnalysisData.platform,
      'Platform should match'
    );
    t.equal(
      analysis.username,
      mockAnalysisData.username,
      'Username should match'
    );
    t.equal(
      analysis.crediScore,
      mockAnalysisData.crediScore,
      'Credi score should match'
    );
...
tap.test(
  'AnalysisRepository.create should allow multiple analyses for same profile URL',
  async (t) => {
    ...
    t.equal(
      first.profileUrl,
      second.profileUrl,
      'Both should have same profile URL'
    );
  }
);

tap.test(
  'AnalysisRepository.findLatestByProfileUrl should find latest analysis by profile URL',
  async (t) => {
    const first = await AnalysisRepository.create(mockAnalysisData);
    ...
    const found = await AnalysisRepository.findLatestByProfileUrl(
      mockAnalysisData.profileUrl
    );
    ...
    t.equal(
      found?.profileUrl,
      mockAnalysisData.profileUrl,
      'Profile URLs should match'
    );
  }
);

tap.test(
  'AnalysisRepository.findAllByProfileUrl should find all analyses for profile URL',
  async (t) => {
    const first = await AnalysisRepository.create(mockAnalysisData);
    const second = await AnalysisRepository.create(mockAnalysisData);

    const found = await AnalysisRepository.findAllByProfileUrl(
      mockAnalysisData.profileUrl
    );

    t.equal(found.length, 2, 'Should find both analyses');
    t.ok(
      found.some((a) => a.id === first.id),
      'Should include first analysis'
    );
    t.ok(
      found.some((a) => a.id === second.id),
      'Should include second analysis'
    );
  }
);

tap.test(
  'AnalysisRepository.findLatestByProfileUrl should return null for non-existent profile URL',
  async (t) => {
    const found = await AnalysisRepository.findLatestByProfileUrl(
      'https://twitter.com/nonexistent'
    );
...
tap.test(
  'AnalysisRepository.findValidLatestByProfileUrl should find valid non-expired analysis',
  async (t) => {
    ...
    const found = await AnalysisRepository.findValidLatestByProfileUrl(
      mockAnalysisData.profileUrl
    );
    ...
    t.equal(
      found?.id,
      validAnalysis.id,
      'Should return the valid analysis, not expired one'
    );
  }
);

tap.test(
  'AnalysisRepository.findExpired should find expired analyses',
  async (t) => {
    const expiredData = {
      ...mockAnalysisData,
    ...
    t.ok(
      expired.some((a) => a.profileUrl === expiredData.profileUrl),
      'Should include the expired analysis'
    );
  }
);
...
tap.test(
  'Caching: findValidLatestByProfileUrl should return null when no valid cache exists',
  async (t) => {
    const found = await AnalysisRepository.findValidLatestByProfileUrl(
      'https://twitter.com/nocache'
    );
...
tap.test(
  'Caching: findValidLatestByProfileUrl should return cached analysis within 24 hours',
  async (t) => {
    const cacheData = {
      ...mockAnalysisData,
...
tap.test(
  'Caching: findValidLatestByProfileUrl should not return expired cache',
  async (t) => {
...
tap.test(
  'Caching: should return most recent valid cache when multiple exist',
  async (t) => {
    ...
    const found =
      await AnalysisRepository.findValidLatestByProfileUrl(profileUrl);

    t.ok(found, 'Should find cached analysis');
    t.equal(
      found?.id,
      newerCreated.id,
      'Should return the most recent valid cache'
    );
    t.equal(
      found?.crediScore,
      9.5,
      'Should return the newer analysis with different score'
    );
  }
);
...
tap.test('Caching: should handle normalized URLs consistently', async (t) => {
  ...
  const foundExact =
    await AnalysisRepository.findValidLatestByProfileUrl(baseUrl);
